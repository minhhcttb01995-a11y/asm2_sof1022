...
public class CandidateController : Controller
{
        ...
        IFormFile? avatarFile)
    {
        ...
        // Vừa cập nhật xong thông tin cá nhân — kiểm tra xem hồ sơ đã đủ để
        // tự động chuyển tài khoản từ "Chờ xác thực" sang "Đang hoạt động" chưa.
        var userForActivation = profile.User ?? await _db.Users.FindAsync(CurrentUserId);
    ...
    public async Task<IActionResult> UploadCv(IFormFile cvFile)
    {
        if (cvFile == null || cvFile.Length == 0)
        {
            ...
            return RedirectBackOrTo("CvManager");
        }
        ...
        try
        {
            ...
            // Vừa có CV đầu tiên — kiểm tra xem hồ sơ đã đủ để tự động chuyển
            // tài khoản từ "Chờ xác thực" sang "Đang hoạt động" chưa.
            var userForActivation = await _db.Users.FindAsync(CurrentUserId);
            if (await ProfileCompletionHelper.TryAutoActivateAsync(_db, userForActivation))
                await _db.SaveChangesAsync();
    ...
    public async Task<IActionResult> MarkRead(int notifId)
    {
        ...
        return RedirectBackOrTo("Notifications");
    }
    ...
    public async Task<IActionResult> AiCvBuilder(int? jobId)
    {
        var profile = await _db.CandidateProfiles
            .Include(p => p.User)
        ...
        var model = new JobConnect.ViewModels.AiCvBuilderViewModel
        {
            ...
            // Prefill kỹ năng sẵn có từ hồ sơ (nếu có) để người dùng không phải gõ lại từ đầu.
            Skills = profile?.CandidateSkills != null && profile.CandidateSkills.Any()
                ? string.Join(", ", profile.CandidateSkills.Select(cs => cs.Skill?.Name).Where(n => !string.IsNullOrWhiteSpace(n)))
                : "",
        ...
        // Nếu người dùng bấm "Tạo CV cho việc này" từ trang chi tiết 1 tin tuyển dụng
        if (jobId.HasValue)
        {
            var job = await _db.JobPosts.Include(j => j.Employer)
                .FirstOrDefaultAsync(j => j.JobId == jobId.Value);
            if (job != null)
            {
                model.TargetJobId = job.JobId;
                model.TargetJobTitle = job.Title;
                model.TargetCompanyName = job.Employer?.CompanyName;
                if (string.IsNullOrWhiteSpace(model.JobTitle)) model.JobTitle = job.Title;
            }
        }
    ...
    public async Task<IActionResult> AiCvBuilder(JobConnect.ViewModels.AiCvBuilderViewModel model)
    {
        ...
        // Nếu form đang "may đo" theo 1 job cụ thể (TargetJobId được giữ qua hidden field),
        // load lại job từ DB (KHÔNG tin dữ liệu title/description do client tự gửi lên) rồi
        // đưa vào request để GeminiService biết cách nhấn mạnh nội dung.
        if (model.TargetJobId.HasValue)
        {
            var job = await _db.JobPosts.Include(j => j.Employer)
                .FirstOrDefaultAsync(j => j.JobId == model.TargetJobId.Value);
            if (job != null)
            {
                model.TargetJobTitle = job.Title;
                model.TargetCompanyName = job.Employer?.CompanyName;
                request.TargetJobTitle = job.Title;
                request.TargetJobDescription = job.Description;
                request.TargetJobRequirements = job.Requirements;
                request.TargetCompanyName = job.Employer?.CompanyName;
            }
        }
