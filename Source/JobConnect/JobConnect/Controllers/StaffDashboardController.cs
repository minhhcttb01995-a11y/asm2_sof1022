...
// [FIX-LOGIC] Trước đây là [Authorize(Roles = "Staff,Admin")] — cho Admin vào XEM được
// khu vực này, nhưng GetCurrentStaffAsync() tra theo bảng Staff (Admin không có dòng nào
// ở đó) nên hầu hết action (duyệt/đổi trạng thái...) sẽ trả về 401 Unauthorized nếu Admin
// bấm vào. Thu hẹp lại chỉ cho Staff, vì Admin đã có sẵn các action tương đương riêng
// trong AdminController (SetJobStatus, ChangeJobPostStatus, Companies, Jobs...).
...
public class StaffDashboardController : Controller
{
    ...
    public StaffDashboardController(AppDbContext context, ILogger<StaffDashboardController> logger, IStatusCatalogService statusSvc, ICodeGeneratorService codeGen, IEmailService emailSvc, IJobModerationService moderationSvc)
    {
        _context = context;
        _logger = logger;
    ...
    // Gửi thông báo "có tin tuyển dụng mới" cho tất cả ứng viên đang theo dõi công ty
    // đăng tin này. RelatedId = JobId để khi bấm vào thông báo, ứng viên được dẫn
    // thẳng tới trang chi tiết tin tuyển dụng đó.
    private async Task NotifyFollowersOfNewJobAsync(JobPost job)
    {
        var followerUserIds = await _context.CompanyFollows
        ...
        var companyName = job.Employer?.CompanyName ?? "Công ty bạn đang theo dõi";
    ...
    // Khi NHÂN VIÊN (Staff) duyệt một tin tuyển dụng, phải báo cho toàn bộ Admin biết
    // (để Admin nắm được/kiểm tra lại nếu cần). Nếu chính Admin tự duyệt thì KHÔNG cần
    // gửi thông báo này (Admin đã biết vì tự tay làm).
    private async Task NotifyAdminsOfStaffApprovalAsync(JobPost job, Staff approvedByStaff)
    {
        ...
        var staffName = approvedByStaff.ApplicationUser?.FullName ?? approvedByStaff.ApplicationUser?.Email ?? "Nhân viên";

        var notifications = adminUserIds.Select(uid => new Notification
        {
            UserId = uid,
            Title = "Nhân viên vừa duyệt một tin tuyển dụng",
            Content = $"{staffName} đã duyệt tin \"{job.Title}\". Bấm vào để xem chi tiết.",
            Type = "System",
            RelatedId = job.JobId,
            IsRead = false,
            CreatedAt = DateTime.Now
    ...
    public async Task<IActionResult> ToggleUserStatus(int userId, string? returnUrl)
    {
        ...
        if (currentStaff.ApplicationUserId == user.UserId)
        {
            ...
            return RedirectBackOrTo(user.Role == "Candidate" ? "Candidates" : "Employers");
        }
    ...
    public async Task<IActionResult> ToggleEmployerLock(int employerId, string? returnUrl)
    {
        ...
        // [FIX] Đồng bộ Status của Employer + User liên kết theo IsLocked, nếu không
        // AccountController.Login (chỉ tra theo Status) sẽ không chặn được tài khoản này.
        employer.Status = employer.IsLocked ? "Banned" : "Active";
        if (employer.User != null)
        {
            employer.User.Status = employer.Status;
        ...
        if (!string.IsNullOrEmpty(returnUrl)) return Redirect(returnUrl);
        return RedirectBackOrTo("Employers");
    }
    ...
    {
        // Mặc định hiển thị công ty "Chờ duyệt" (Pending) trước. Chỉ ghi đè khi
        // không có tham số status nào được truyền lên (vào trang lần đầu / bấm
        // "Đặt lại"); nếu người dùng chọn "Tất cả trạng thái" trên UI, status sẽ
        // là "All" (giá trị tường minh, tránh mọi nhầm lẫn null/rỗng) và không lọc gì cả.
        status ??= "Pending";
        ...
        ViewBag.Page = page;
        ViewBag.Status = status;
        // [FIX] Trước đây dùng StatusEntityTypes.Company — khác với EntityType "Employer" mà
        // trang /Admin/Companies và toàn bộ hệ thống đang dùng cho cùng bảng Employer, khiến
        // trạng thái mới tạo ở mục "Công ty" không hiện ra ở đây (và ngược lại). Hợp nhất về Employer.
        ViewBag.CompanyStatuses = await _statusSvc.GetActiveByEntityTypeAsync(StatusEntityTypes.Employer);
    ...
    public async Task<IActionResult> ApproveCompany(int employerId)
    {
        ...
        return RedirectBackOrTo("PendingCompanies");
    }
    ...
    {
        // Mặc định hiển thị tin "Chờ duyệt" (Pending) trước — xem giải thích ở
        // PendingCompanies() phía trên. Khi người dùng chọn "Tất cả trạng thái"
        // trên UI, status sẽ là "All" và không lọc gì cả.
        status ??= "Pending";
        ...
        // Tự động chấm AI cho toàn bộ tin ĐANG HIỂN THỊ trên trang này (không cần Staff bấm
        // từng tin). ModerateManyAsync tự kiểm tra cache theo ContentHash bên trong — tin nào
        // nội dung chưa đổi từ lần chấm trước sẽ không tốn thêm lượt gọi AI nào.
    ...
    public async Task<IActionResult> ApproveJob(int jobId)
    {
        ...
        // Thông báo cho ứng viên đang theo dõi công ty này biết có tin tuyển dụng mới
        await NotifyFollowersOfNewJobAsync(job);

        // Nhân viên duyệt tin → phải báo cho Admin biết
        await NotifyAdminsOfStaffApprovalAsync(job, currentStaff);
        ...
        return RedirectBackOrTo("PendingJobs");
    }
    ...
    public async Task<IActionResult> ProcessReport(int reportId, string action, string? note, bool? hideJob)
    {
        ...
        return RedirectBackOrTo("Reports");
    }
    ...
    public async Task<IActionResult> AssignTicket(int ticketId)
    {
        ...
        return RedirectBackOrTo("TicketDetail", new { id = ticketId });
    }
    ...
    public async Task<IActionResult> CloseTicket(int ticketId)
    {
        ...
        return RedirectBackOrTo("Tickets");
    }
    ...
    public async Task<IActionResult> Categories(string? type, int page = 1)
    {
        ...
        ViewBag.Page = page;
        ViewBag.Type = type;
        // Dùng để đổ dropdown "Danh mục cha" trong form Tạo/Sửa danh mục (chỉ có ý nghĩa
        // khi loại danh mục là "JobType" — gán JobType vào đúng ngành Industry).
        ViewBag.AllIndustries = await _context.Categories.Where(c => c.Type == "Industry").ToListAsync();
    ...
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> CreateCategory(string name, string type, string slug, string? description, int? parentId)
    {
        ...
        // [FIX-LOGIC] Trước đây form KHÔNG có ô "Danh mục cha" nên mọi danh mục con
        // (Type=JobType) tạo mới đều có ParentId = NULL — không gắn được vào ngành nghề
        // (Industry) nào, khiến bộ lọc "Ngành nghề" ở trang tìm việc không nhận diện được
        // các tin dùng danh mục đó. parentId chỉ có ý nghĩa khi type == "JobType".
        _context.Categories.Add(new Category
        {
            ...
            Slug = slug,
            Description = description,
            ParentId = type == "JobType" ? parentId : null,
            CreatedAt = DateTime.Now
        });
        await _context.SaveChangesAsync();

        TempData["Success"] = "Đã tạo danh mục.";
        return RedirectBackOrTo("Categories", new { type });
    }

    // POST: StaffDashboard/EditCategory
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> EditCategory(int categoryId, string name, string type, string slug, string? description, int? parentId)
    {
        ...
        category.ParentId = type == "JobType" ? parentId : null;
    ...
    {
        // Mặc định hiển thị bài viết "Chờ duyệt" (Pending) trước — xem giải
        // thích chi tiết ở action PendingJobs/PendingCompanies phía dưới.
        status ??= "Pending";
        ...
        {
            query = query.Where(b => !b.IsPublished && b.Status != "Pending");
        }
    ...
    public async Task<IActionResult> SaveBlog(int? postId, string title, string slug, string? excerpt, string? coverUrl, string content, string status)
    {
        ...
        // Bài do Nhân viên (Staff) soạn mà chưa xuất bản luôn ở trạng thái
        // "Pending" (Chờ duyệt) để Admin thấy ngay trong danh sách mặc định của
        // trang Quản lý Blog, tương tự cơ chế duyệt Tin tuyển dụng/Công ty.
        post.Status = post.IsPublished ? "Published" : "Pending";
    ...
    public async Task<IActionResult> SendNotification(string title, string content, string target, string? specificEmails)
    {
        ...
        if (target == "Specific")
        {
            // Gửi cho MỘT hoặc NHIỀU người cụ thể: nhân viên nhập email, phân tách nhau
        ...
        _context.Notifications.AddRange(notifications);
        await _context.SaveChangesAsync();
        await LogActivityAsync(currentStaff, "SEND_NOTIFICATION", $"Gửi thông báo đến {targetLabel}: {notifications.Count} người");
    ...
    public async Task<IActionResult> UpdateProfile(string fullName, string phone, string email, string? gender)
    {
        ...
        return RedirectBackOrTo("Profile");
    }
  ...
  <tr><td style='background:#0f172a;padding:16px 40px;border-top:1px solid #1e293b;text-align:center;'>
    ...
    public async Task<IActionResult> ChangeCandidateStatus(int profileId, string newStatus)
    {
        ...
        if (!statusExists)
        {
            ...
            return RedirectBackOrTo("Candidates");
        }
    ...
    public async Task<IActionResult> ChangeEmployerStatus(int employerId, string newStatus)
    {
        ...
        _context.Notifications.Add(new Notification
        {
            ...
            Content = $"Nhân viên đã cập nhật trạng thái công ty thành: {newStatus}.",
            Type = "System",
    ...
    public async Task<IActionResult> ChangeJobStatus(int jobId, string newStatus)
    {
        ...
        if (job.Employer != null)
        {
            _context.Notifications.Add(new Notification
            {
                ...
                Content = $"Nhân viên đã cập nhật trạng thái tin thành: {newStatus}.",
                Type = "System",
                RelatedId = job.JobId,
                IsRead = false,
                CreatedAt = DateTime.Now
        ...
        // Nếu tin vừa chuyển sang "Active" (từ trạng thái khác) thì báo cho ứng viên
        // đang theo dõi công ty này biết có tin tuyển dụng mới.
    ...
    public async Task<IActionResult> ChangeCompanyStatus(int employerId, string newStatus)
    {
        ...
        // Validate status exists in StatusCatalog
        var statusExists = await _context.StatusCatalogs
            .AnyAsync(s => s.EntityType == StatusEntityTypes.Employer && s.Code == newStatus && s.IsActive); // [FIX] đồng bộ với EntityType dùng ở /Admin/Companies
