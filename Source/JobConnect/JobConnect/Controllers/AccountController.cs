...
public class AccountController : Controller
{
    ...
    /// <summary>
    /// [FIX] Nút "Khóa/Mở khóa" nhanh ở trang quản trị Employer chỉ đổi cờ
    /// Employer.IsLocked, KHÔNG đổi Status, nên IsStatusBlockingLoginAsync (chỉ tra
    /// theo Status) không phát hiện được. Kiểm tra thêm cờ này riêng cho role Employer.
    /// </summary>
    private async Task<bool> IsEmployerLockedAsync(int userId)
    {
        return await _db.Employers.AnyAsync(e => e.UserId == userId && e.IsLocked);
    }
    ...
    public async Task<IActionResult> Login(LoginViewModel model, string? returnUrl = null)
    {
        ...
        try
        {
            ...
            // Chặn đăng nhập nếu tài khoản đã bị xóa mềm (DeletedAt != null).
            if (user.DeletedAt != null)
            {
                ModelState.AddModelError("", "Tài khoản của bạn đã bị khóa hoặc xóa. Vui lòng liên hệ quản trị viên.");
                if (IsAjaxRequest())
                {
                    Response.StatusCode = 400;
                    return PartialView("_LoginModalPartial", model);
            ...
            var isBlockedByCatalog = await IsStatusBlockingLoginAsync(user.Role, user.Status);

            // [FIX] Ngoài Status, còn phải chặn nếu Employer đang bị khoá nhanh (IsLocked = true)
            // vì trước đây nút "Khóa" không đổi Status nên account vẫn login được.
            var isBlockedByEmployerLock = user.Role == "Employer" && await IsEmployerLockedAsync(user.UserId);

            if (isBlockedByCatalog || isBlockedByEmployerLock)
            {
    ...
    public async Task<IActionResult> GoogleResponse(string? returnUrl = null)
    {
        ...
        if (user == null)
        {
            ...
            // LƯU Ý: KHÔNG set Status = "Active" ngay — cũng như đăng ký thường
            // (email/OTP), tài khoản mới qua Google phải ở "Pending" (Chờ xác
            // thực) cho tới khi ứng viên bổ sung đầy đủ hồ sơ + CV, lúc đó
            // ProfileCompletionHelper.TryAutoActivateAsync sẽ tự chuyển Active.
            user = new User
            {
                Email = email,
                FullName = name ?? email,
            ...
            // Thông báo chào mừng + nhắc hoàn thiện hồ sơ (giống luồng đăng ký
            // thường), vì hồ sơ vừa tạo còn trống, chưa thể ứng tuyển ngay.
            ...
            await _db.NotifyAdminsAsync(
                ...
                content: $"Ứng viên \"{user.FullName}\" ({user.Email}) vừa đăng ký tài khoản qua Google.",
                type: "NewCandidate",
        ...
        // [FIX] Bổ sung kiểm tra Employer.IsLocked (xem ghi chú tại IsEmployerLockedAsync)
        var isBlockedByEmployerLockGoogle = user.Role == "Employer" && await IsEmployerLockedAsync(user.UserId);
        if (await IsStatusBlockingLoginAsync(user.Role, user.Status) || isBlockedByEmployerLockGoogle)
        {
    ...
    public async Task<IActionResult> ResetPassword(string email, int tokenId, string newPassword, string confirmPassword)
    {
        ...
        var token = await _db.PasswordResetTokens
            .FirstOrDefaultAsync(t => t.Id == tokenId && t.Email == email.Trim().ToLower() && !t.IsUsed && t.ExpiresAt > DateTime.Now);
...
<table width='520' cellpadding='0' cellspacing='0' style='background:#111827;border-radius:16px;border:1px solid #1e293b;overflow:hidden;'>
  ...
  <tr><td style='background:#0f172a;padding:16px 40px;border-top:1px solid #1e293b;text-align:center;'>
    ...
    public async Task<IActionResult> VerifyEmailOtp(string email, string otp, string purpose)
    {
        ...
        if (purpose == "register")
        {
            // Không chuyển thẳng sang "Active" nữa: tài khoản vẫn ở "Pending"
            // (Chờ xác thực) cho tới khi hồ sơ (Candidate: thông tin cá nhân + CV;
            // Employer: thông tin cá nhân + công ty) được bổ sung ĐẦY ĐỦ. Nếu ngay
            // lúc này hồ sơ đã đủ (hiếm khi xảy ra) thì kích hoạt luôn.
            await ProfileCompletionHelper.TryAutoActivateAsync(_db, user);
        }
