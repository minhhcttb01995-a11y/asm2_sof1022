...
public class StaffController : Controller
{
    ...
    public async Task<IActionResult> ChangeStatus(int id, string newStatus)
    {
        try
        {
            ...
            // [FIX] Trước đây luôn ghi cứng "Banned"/"Active" vào User.Status thay vì
            // ghi đúng Code (newStatus) mà Admin chọn, khiến User.Status và Staff.Status
            // lệch nhau và việc chặn đăng nhập chỉ "ăn may" khi có đúng 1 trạng thái tên
            // "Banned". Giờ ghi thẳng đúng Code — AccountController sẽ tự tra BlocksLogin
            // theo đúng Code này khi login.
            if (staff.ApplicationUser != null)
            {
                staff.ApplicationUser.Status = newStatus;
    ...
    // Chạy 1 câu lệnh DELETE/UPDATE, nhưng nếu bảng KHÔNG TỒN TẠI trong DB thực tế
    // (lỗi SQL Server 208 "Invalid object name") thì bỏ qua thay vì làm hỏng cả giao dịch.
    ...
    public async Task<IActionResult> PermanentDelete(int id)
    {
        ...
        var userId = staff.ApplicationUser?.UserId;

        using var tx = await _context.Database.BeginTransactionAsync();
        try
        {
            // Xóa toàn bộ dữ liệu liên quan (log hoạt động, báo cáo đã xử lý, ticket đã nhận,
            // và dữ liệu tài khoản User đi kèm) trước khi xóa bản ghi Staff/User.
            await TryDeleteAsync($@"
                DELETE FROM ActivityLogs WHERE StaffId = {staffId}");
            await TryDeleteAsync($@"
                UPDATE Reports SET ProcessedByStaffId = NULL WHERE ProcessedByStaffId = {staffId}");
            await TryDeleteAsync($@"
                UPDATE SupportTickets SET AssignedToStaffId = NULL WHERE AssignedToStaffId = {staffId}");
            ...
            if (userId != null)
            {
                ...
                await TryDeleteAsync($@"
                    DELETE FROM SavedJobs WHERE UserId = {userId}");
                await TryDeleteAsync($@"
                    DELETE FROM Reports WHERE ReporterId = {userId}");
                await TryDeleteAsync($@"
                    DELETE FROM BlogPosts WHERE AuthorID = {userId}");
                await TryDeleteAsync($@"
                    DELETE FROM SystemLogs WHERE UserId = {userId}");
                await TryDeleteAsync($@"
                    DELETE FROM SupportTickets WHERE UserId = {userId}");
                await TryDeleteAsync($@"
                    DELETE FROM PasswordResetTokens WHERE Email = {staff.ApplicationUser!.Email}");
            }

            await _context.Database.ExecuteSqlInterpolatedAsync($@"DELETE FROM Staff WHERE Id = {staffId}");
            if (userId != null)
                await _context.Database.ExecuteSqlInterpolatedAsync($@"DELETE FROM Users WHERE UserId = {userId}");
        ...
        catch (Exception ex)
        {
            ...
            TempData["Error"] = $"Không thể xóa vĩnh viễn nhân viên {staffName}: {ex.Message}";
            return RedirectBackOrTo(nameof(Index));
        }
