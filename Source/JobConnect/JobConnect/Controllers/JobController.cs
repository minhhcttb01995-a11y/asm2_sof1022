    ...
    public class JobController : Controller
    {
        ...
        public async Task<IActionResult> Apply(int jobId, int cvId, string? coverLetter)
        {
            ...
            try
            {
                ...
                // Ứng viên phải có hồ sơ ĐẦY ĐỦ thông tin cá nhân VÀ ít nhất 1 CV
                // thì mới được phép ứng tuyển — không chỉ cần tồn tại hồ sơ trống.
                bool hasCv = profile?.CvFiles.Any() == true;
                if (!ProfileCompletionHelper.IsCandidateProfileComplete(profile, hasCv))
                {
                    const string msg = "Bạn cần bổ sung đầy đủ thông tin cá nhân và CV trước khi ứng tuyển.";

                    // Không chỉ báo lỗi trên popup — còn tạo THÔNG BÁO (chuông) để nhắc lại,
                    // tránh trùng lặp thông báo nếu họ vừa được nhắc trong 1 giờ gần đây.
                    await ProfileCompletionHelper.RemindIncompleteAsync(
                        _db, userId,
                        title: "Hồ sơ ứng viên chưa đầy đủ",
                        content: "Bạn cần bổ sung đầy đủ thông tin cá nhân (họ tên, số điện thoại, địa chỉ, vị trí mong muốn) và tải lên ít nhất 1 CV trước khi có thể ứng tuyển.");
