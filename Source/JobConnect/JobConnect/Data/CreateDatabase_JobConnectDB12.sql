/* =========================================================
   JobConnectDB12 - XOA VA TAO LAI HOAN TOAN TU DAU
   CANH BAO: Script nay se XOA SACH du lieu cu trong JobConnectDB12
...
IF DB_ID('JobConnectDB12') IS NOT NULL
BEGIN
    ALTER DATABASE JobConnectDB12 SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE JobConnectDB12;
END
GO
 
CREATE DATABASE JobConnectDB12;
GO
...
/* ================= SEED STATUS CATALOG ================= */
INSERT INTO StatusCatalog (EntityType, Code, Name, ColorClass, Description, IsActive, IsSystem) VALUES
-- Candidate
('Candidate', 'Active', N'Đang hoạt động', 'bg-green-100 text-green-700', NULL, 1, 1),
('Candidate', 'Pending', N'Chờ xác thực', 'bg-yellow-100 text-yellow-700', NULL, 1, 1),
('Candidate', 'Banned', N'Đã khóa', 'bg-red-100 text-red-700', NULL, 1, 1),
-- Employer
('Employer', 'Active', N'Đang hoạt động', 'bg-green-100 text-green-700', NULL, 1, 1),
('Employer', 'Pending', N'Chờ duyệt', 'bg-yellow-100 text-yellow-700', NULL, 1, 1),
('Employer', 'Verified', N'Đã xác minh', 'bg-blue-100 text-blue-700', NULL, 1, 1),
('Employer', 'Banned', N'Đã khóa', 'bg-red-100 text-red-700', NULL, 1, 1),
-- Staff
('Staff', 'Active', N'Đang làm việc', 'bg-green-100 text-green-700', NULL, 1, 1),
('Staff', 'Locked', N'Đã khóa', 'bg-red-100 text-red-700', NULL, 1, 1),
('Staff', 'Deleted', N'Đã xóa', 'bg-gray-100 text-gray-700', NULL, 1, 1),
-- Company
('Company', 'Active', N'Đang hoạt động', 'bg-green-100 text-green-700', NULL, 1, 1),
('Company', 'Pending', N'Chờ duyệt', 'bg-yellow-100 text-yellow-700', NULL, 1, 1),
('Company', 'Verified', N'Đã xác minh', 'bg-blue-100 text-blue-700', NULL, 1, 1),
('Company', 'Banned', N'Đã khóa', 'bg-red-100 text-red-700', NULL, 1, 1),
-- JobPost
('JobPost', 'Active', N'Đã duyệt', 'bg-green-100 text-green-700', NULL, 1, 1),
('JobPost', 'Pending', N'Chờ duyệt', 'bg-yellow-100 text-yellow-700', NULL, 1, 1),
('JobPost', 'Rejected', N'Đã từ chối', 'bg-red-100 text-red-700', NULL, 1, 1),
('JobPost', 'Banned', N'Đã khóa', 'bg-red-100 text-red-700', NULL, 1, 1),
('JobPost', 'Draft', N'Bản nháp', 'bg-purple-100 text-purple-700', NULL, 1, 1),
-- BlogPost
('BlogPost', 'Published', N'Đã xuất bản', 'bg-green-100 text-green-700', NULL, 1, 1),
('BlogPost', 'Draft', N'Bản nháp', 'bg-yellow-100 text-yellow-700', NULL, 1, 1),
('BlogPost', 'Pending', N'Chờ duyệt', 'bg-blue-100 text-blue-700', NULL, 1, 1);
GO
 
-- Dua DB ve che do multi-user binh thuong
ALTER DATABASE JobConnectDB12 SET MULTI_USER;
GO
...
SELECT 'Employers.UpdatedAt',
       CASE WHEN EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Employers') AND name = 'UpdatedAt')
            THEN 'OK' ELSE 'MISSING' END
UNION ALL
SELECT 'Employers.Status',
       CASE WHEN EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Employers') AND name = 'Status')
            THEN 'OK' ELSE 'MISSING' END
UNION ALL
SELECT 'Staff.CCCD',
...
SELECT 'Staff.Gender',
       CASE WHEN EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Staff') AND name = 'Gender')
            THEN 'OK' ELSE 'MISSING' END
UNION ALL
SELECT 'Staff.Status (NVARCHAR)',
       CASE WHEN EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('Staff') AND name = 'Status' AND system_type_id = 231)
            THEN 'OK' ELSE 'MISSING' END
UNION ALL
