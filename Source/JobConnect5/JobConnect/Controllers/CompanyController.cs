...
public class CompanyController : Controller
{
    ...
    // GET /Company
    public async Task<IActionResult> Index(string? keyword, string? industry, string? location, int page = 1)
    {
        ...
        // [FIX-LOGIC] Trước đây dropdown "Địa điểm" trên UI chỉ để trang trí — controller
        // không nhận tham số location nên chọn gì cũng không lọc được gì. Lọc theo địa chỉ
        // công ty có chứa tên địa điểm đã chọn (Address là chuỗi tự do, không tách sẵn theo
        // thành phố, nên dùng Contains tương tự cách lọc từ khóa).
        if (!string.IsNullOrWhiteSpace(location))
            q = q.Where(e => e.Address != null && e.Address.Contains(location));
        ...
        // [FIX-LOGIC] Trước đây hardcode cứng 3 thành phố trong view, giờ lấy động từ danh
        // mục Type="Location" giống hệt trang tìm việc — có bao nhiêu địa điểm trong danh
        // mục thì dropdown hiện đủ, tạo thêm địa điểm mới cũng tự nhận không cần sửa code.
        ViewBag.Locations = await _db.Categories.Where(c => c.Type == "Location").ToListAsync();
        return View(results);
    }
