...
public static class StatusEntityTypes
{
    ...
    // [FIX] Đã bỏ hằng số "Company" riêng biệt: trước đây "Company" và "Employer" là 2
    // EntityType khác nhau nhưng cùng dùng để lưu trạng thái cho MỘT bảng dữ liệu (Employer),
    // khiến trạng thái tạo dưới nhãn "Công ty" không hiện ra ở các màn hình đọc theo "Employer"
    // (và ngược lại). Giữ lại hằng số Company (trỏ về cùng giá trị Employer) chỉ để KHÔNG phá
    // vỡ những chỗ code cũ còn tham chiếu StatusEntityTypes.Company, nhưng nó không còn xuất
    // hiện như một lựa chọn riêng trong danh sách All/Labels nữa.
    public const string Company = Employer;
    public const string JobPost = "JobPost";
    public const string BlogPost = "BlogPost";

    public static readonly string[] All = { Candidate, Employer, Staff, JobPost, BlogPost };
